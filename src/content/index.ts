import { MessageType, BaseMessage, ApiDataPayload } from '../shared/types/messages';
...
class ContentScript {
  ...
  private async initialize(): Promise<void> {
    ...
    if (!this.isAuditPage()) {
      logger.info('Not on audit page, but interceptor active for completion detection');
      return;
    }
  ...
  private handleApiData(payload: ApiDataPayload): void {
    ...
    if (payload.extractedInfo) {
      ...
      logger.warn('API data captured but no extractedInfo found', { endpoint: payload.endpoint });
    }
  }
  ...
  private startTracking(): void {
    ...
    if (!qaOperationId) {
      ...
      if (auditIndex !== -1 && auditIndex < pathParts.length - 1) {
        ...
        if (nextPart && nextPart !== 'results' && nextPart.match(/^[a-f0-9]{24}$/)) {
    ...
    chrome.runtime.sendMessage({
      ...
      if (response?.resumed) {
        logger.info('Resumed existing task tracking', { qaOperationId, response });
      } else {
  ...
  private handleTaskCompleted(): void {
    logger.info('Task completed (pre-final stage)', {
      ...
      message: 'Saving completion time but timer continues - waiting for transition or new task'
    });
  ...
  private handleTaskTransitioned(): void {
    ...
    chrome.runtime.sendMessage({
      ...
      logger.info('Tracking stopped successfully after transition', { response });
    }).catch(error => {
  ...
  private handleBackgroundMessage(message: BaseMessage, sendResponse: Function): void {
    ...
    switch (message.type) {
      ...
      case MessageType.TRACKING_STATE_CHANGED:
        ...
        logger.info('Tracking state changed', { enabled: this.trackingEnabled });
        sendResponse({ success: true });
        break;
    ...
    (window as any).qcAuditDebug = {
      ...
      checkForInterceptorScript: () => {
        ...
        scripts.forEach(script => {
          if (script.textContent && script.textContent.includes('[QC-Interceptor]')) {
            found = true;
          }
        });
        return found;
